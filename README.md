...
This project is based on the course by **Ayush Singh**, Data Scientist at Replayed and Founder of SecondBrainLabs. Ayush has led several products in the creators' economy and worked as an MLOps engineer on one of the fastest-growing MLOps frameworks, [ZenML](https://zenml.io/).

## :snake: Python Requirements

### Setup (WSL on Windows)

This project requires daemon support (for MLflow), which is not available on Windows natively. We use WSL (Windows Subsystem for Linux) instead.
...
#### 2. Set up virtual environment helper functions

Add the following to your WSL `~/.bashrc` to manage virtual environments:

```bash
export WORKON_HOME=$HOME/.virtualenvs
mkvirtualenv() { python3 -m venv "$WORKON_HOME/$1" && source "$WORKON_HOME/$1/bin/activate"; }
workon() { source "$WORKON_HOME/$1/bin/activate"; }
lsvirtualenv() { ls "$WORKON_HOME"; }
rmvirtualenv() { rm -rf "$WORKON_HOME/$1"; }
```
...
| `workon <name>`       | Activate a virtual environment     |
...
```bash
cd /mnt/d/LocalProjects/mlops-projects-course/customer-satisfaction-mlops-main/customer-satisfaction-mlops-main
```

Create the environment and install packages:

```bash
mkvirtualenv customer-satisfaction-app
